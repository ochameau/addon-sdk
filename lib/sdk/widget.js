...
BrowserWindow.prototype = {
  ...
  _insertNodeInToolbar: function BW__insertNodeInToolbar(node) {
    ...
    // On first run (i.e. on install), if widget isn't in any toolbar,
    // add it to the addon-bar
    if (!container && require("./self").loadReason == "install") {
...
WidgetChrome.prototype._paletteObserver = function WC__paletteObserver(mutations) {
  let node = this.node;
  let removedFromPalette = mutations.some(function (m) {
    return (m.removedNodes.length == 1 && m.removedNodes[0] == node);
  });
  // If the widget has been moved from the palette to the toolbar,
  // setup the symbiont and load widget's content
  if (removedFromPalette) {
...
WidgetChrome.prototype.setContent = function WC_setContent() {
  // No need to set/update widget content if the toolbaritem isn't in DOM.
  // That basically means that the widget is in the toolbar palette.
  if (!this.node.parentNode.parentNode) {
    let self = this;
    let { MutationObserver } = this.window;
    let obs = new MutationObserver(this._paletteObserver.bind(this));
    obs.observe(this.node.parentNode, {childList: true});
    this._mutationObserver = obs;
    return;
  }
...
WidgetChrome.prototype.destroy = function WC_destroy(removedItems) {
  ...
  // cleanup symbiont
  // if the widget has been moved to the palette, the symbiont has already
  // been unloaded and deleted
  if (this._symbiont)
    this._symbiont.destroy();
