...
def run(arguments=sys.argv[1:], target_cfg=None, pkg_cfg=None,
    ...
    used_files = None
    if command == "xpi":
      # Use a list() in order to be JSONifiable
      used_files = list(manifest.get_used_files())
    ...
    cfxjs.execute('build-xpi', {
            # Options for FS lookup and manifest generation:
            'rootPath': options.pkgdir, # path to the addon directory
            'sdkPath': env_root, # path to SDK root folder
            # Options needed for xpi building step:
            'templatePath': app_extension_dir,
            'installRdf': str(manifest_rdf),
            'xpiPath': os.path.abspath(xpi_path), # JS expect only full absolute path
            'harnessOptions': harness_options.copy(), # will be mostly computed by JS side in manifest building step
                                                      # but some attributes may still be generated by python
                                                      # like `logFile` and `resultFile`
            'extraHarnessOptions': extra_harness_options,
            'limitTo': used_files # will be computed by new JS manifest implementation
        }, options.binary)
